...
eq_num = 0 #from 0 to 21
mat_p_num = 0 #from 0 to 299
#N = 10000
#Est_mu = 210000000
for sc_num in range(5):
    ...
    Est_list = lognorm.ppf(asd[0,:],np.log(Est_mu),(Est_cov*Est_mu)/100)
    plt.figure(figsize=(12, 9))
    count, bins, ignored = plt.hist(Est_list[:,], 50, density=True)
    
    Ec_mu = 32000000
    asd1 = np.random.uniform(low=0.0, high =1.0, size=[1,300])
    Ec_cov = 0.33
    Ec_list = norm.ppf(asd1[0,:],Ec_mu,Ec_cov*Ec_mu/100)
    plt.figure(figsize=(12, 9))
    count, bins, ignored = plt.hist(Ec_list[:,], 50, density=True)
    '''
    
    exec(open("./Random_vars.py").read())
    ...
    EQ_rec_ind = EQ_list[eq_num,:]
    EQ_dt = EQ_dt_list[eq_num]
    
    Est = Est_list[mat_p_num]
    
    Ec = Ec_list[mat_p_num]
    ...
    fix(132,0,0,1,0,0,0)
    fix(134,0,0,1,0,0,0)
    section('Elastic', 104, Est, 1.08, 0.51, 0.51, 76923080, 1.0731, 1, 1)
    ...
    for jk in range(len(Nonl_nodes)):
        recorder('Node', '-file', 'Disp_Sc_'+str(sc_num)+'_EQ_'+str(eq_num)+'_M_'+str(mat_p_num)+str(20000+Nonl_nodes[jk])+'_d5.out', '-time','-node', int(20000+Nonl_nodes[jk]), '-dof', 4,5 , 'disp')
        recorder('Element', '-file', 'Element_Sc_'+str(sc_num)+'_EQ_'+str(eq_num)+'_M_'+str(mat_p_num)+str(int(pile_ele[jk]))+'.out',  '-time', '-closeOnWrite', '-ele', int(pile_ele[jk]), 'force' )
    ...
    for jk in range(len(Nonl_nodes)):
        disp5[jk,:,:] = pd.DataFrame(pd.read_csv('Disp_Sc_'+str(sc_num)+'_EQ_'+str(eq_num)+'_M_'+str(mat_p_num)+str(20000+Nonl_nodes[jk])+'_d5.out',delimiter=" ", header = None)).to_numpy() 
        #disp4[jk,:,:] = pd.DataFrame(pd.read_csv('Disp_'+str(10000+Nonl_nodes[jk])+'_d4.out',delimiter=" ", header = None)).to_numpy() 
        ele_res[jk,:,:] = pd.DataFrame(pd.read_csv('Element_Sc_'+str(sc_num)+'_EQ_'+str(eq_num)+'_M_'+str(mat_p_num)+str(int(pile_ele[jk]))+'.out',delimiter=" ", header = None)).to_numpy() 
